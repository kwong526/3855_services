...
def check():
    ...
    try:
        res = requests.get("http://74.235.55.16/receiver/health")
    ...
    try:
        res = requests.get("http://74.235.55.16/storage/health")
        if res.status_code == 200:
            statuses["storage"] = "Up"
        else:
    ...
    try:
        res = requests.get("http://74.235.55.16/processing/health")
        if res.status_code == 200:
